...
const heroBadges = [
  ...
  { icon: 'qr', label: 'QR\nAttendance' },
  { icon: 'building', label: 'Office Duty\nTracking' },
...
const coreFeatures = [
  { title: 'Event Attendance', desc: 'Officers check in through GPS verification tied to the event location.', icon: 'calendar' },
  { title: 'QR Check-In', desc: 'Fast staff-assisted QR code scanning for delegated attendance.', icon: 'qr' },
  { title: 'Office Duty', desc: 'Daily tracking and reporting for assigned officer office hours.', icon: 'building' },
  { title: 'Offline Support', desc: 'Attendance is queued locally and synced automatically when reconnected.', icon: 'wifi' },
  { title: 'Attendance Export', desc: 'Download CSV reports for any event, date range, or officer.', icon: 'csv' },
  { title: 'Audit Trail', desc: 'Every check-in is time-stamped and immutable for transparent records.', icon: 'audit' },
]
</script>
...
<template>
  <div>
    ...
    <section class="relative text-white overflow-hidden min-h-[580px] flex items-center">
      <!-- Campus background photo — ISUFST Dingle Campus aerial -->
      <div
      ...
      <!-- 2. Strong left-to-right: navy saturates left for text legibility, fades right to reveal campus -->
      <div
      ...
      />
      <!-- 3. Bottom vignette — blends into next section -->
      <div
      ...
      <div class="relative max-w-7xl mx-auto px-4 md:px-6 py-20 md:py-28 w-full">
        <div class="max-w-2xl">
          ...
          <!-- Feature icon badges -->
          <div class="grid grid-cols-3 sm:grid-cols-6 gap-2.5 max-w-xl">
            <div
              v-for="badge in heroBadges"
              :key="badge.label"
              class="flex flex-col items-center gap-1.5 p-2.5 rounded-xl bg-white/8 border border-white/15 backdrop-blur-sm hover:bg-white/15 transition-colors cursor-default"
            >
          ...
          >
            ...
            >
              ...
              <div
                ...
                :class="activeFaq === i ? 'bg-navy border-navy' : 'bg-transparent'"
              >
                <svg
                  class="w-3 h-3 transition-transform duration-200"
                  :class="activeFaq === i ? 'text-white rotate-180' : 'text-slate'"
